...
act_power_all <- household_all %>% 
  ...
  mutate( Date = dmy(Date)) %>%  #getting daily data into date object
  mutate( Time = hms(Time)) %>%  #getting hourly data into 'date' object
...
#could also do this with nested for loops, but this is easier
...
#usually use group_by() before summarise() function -- tells it what to do with the rows that you are grouping

ggplot(act_power_hourly, aes(x=Date,y=hourly_mean_active_power)) +
  geom_line() +
  ylab("Average Hourly Household Active Power")

#Creating a data frame with daily observations -- do something very similar!
...
ts_act_power_hourly <- msts(act_power_hourly$hourly_mean_active_power, 
                            seasonal.periods =c(24,168,8766), #24 (24 hrs); 168 (repeat after 168 weeks); 8766 (repeat year after this)
                            start=c(2006,12,16)) #need to specify year, month, and day for daily data

ts_act_power_daily <- msts(act_power_daily$daily_mean_active_power, 
                           seasonal.periods =c(7,365.25), #7 accounts for weekly pattern; 365.25 accounts for daily pattern
                           start=c(2006,12,16))
...
```{r message=FALSE, warning=FALSE}
#create a subset for TRAINING purpose - could create a new msts that just has one fewer years than the full dataset; could use the window function; can also use the subset function 
n_for = 365 #planning horizon in this case is to forecast out one year of load, so you should leave 365 days out of your analysis so you can forecast with this later
...
# play with K by changing it to K=c(2,2), K=c(2,4), K=c(2,6), etc. The higher the K the longer it will take to converge, because R will try more models. -- we are adding the fourier coefficients as exogenous variables to our non-seasonal ARIMA
...
#xreg argument to add external regressors to the ARIMA model - calling the fourier() function and telling it how many terms you think you need to model the seasonality -- for the daily patterns, maybe you think you just need two terms -- one to represent weekdays and another for weekend; and then maybe you need up to 12 to represent the annual patterns (months), but you could've put four if you thought quarterly was all that was important, etc.
...
**T**rigonometric terms added (the first T) to represent the seasonal component of our dataset
...
NN_for <- forecast(NN_fit, h=365,xreg=fourier(ts_act_power_daily_train, 
                                          K=c(2,12),h=365)) #for forecasting competition, change this to 2 and 6 or 2 and 4 to see what it does to your forecast
