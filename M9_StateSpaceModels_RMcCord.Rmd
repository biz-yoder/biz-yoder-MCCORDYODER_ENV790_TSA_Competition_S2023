...
#if you run in console it might tell you which model is best -- try this to know that R thinks AAM fits best
```
...
checkresiduals(SS_seas) #mean is close to zero, normality is okay -- but we can see in the ACF that our errors are strongly correlated -- need to deal with this by playing with the variance parameter values

SS_seas2 <- StructTS(ts_price,
                    type="BSM",fixed=c(0,0.001,0.3,NA))   #this function has convergence issues if you try to fix all of the parameters -- the function needs some freedome to try and fit things -- keep these numbers relatively small so you don't let your variances get too high

SS_seas2$coef
checkresiduals(SS_seas2) #this is better but not perfect...keep playing with these values until you get something you're happy with 

#Generating forecasts
# StructTS() does not call the forecast() internally so we need one more step
SS_for <- forecast(SS_seas2,h=n_for)
plot(SS_for)

#this model is still trying to assume a linear trend
```
